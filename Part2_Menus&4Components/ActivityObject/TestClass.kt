...
class TestClass() : Parcelable{
    ...
    // 객체가 intent에 저장되는 것이 아닌 객체를 구성하는 변수와 값이 저장된다.
   override fun writeToParcel(p0: Parcel?, p1: Int) {
       p0?.writeInt(data1)
       p0?.writeString(data2)
    }
    ...
    companion object CREATOR : Parcelable.Creator<TestClass> {
        ...
        // getPaclelableExtra 를 호출해서 객체를 추출하려고 하면 자동으로 호출되는 메서드
        // 새로운 객체를 생성하고 parcel에 저장되어 있는 데이터를 추출해 객체의 변수에 담아준다
        // 그 후  생성된 객체를 반환함

        override fun createFromParcel(parcel: Parcel): TestClass {
//            return TestClass(parcel) 31번코드를 풀어쓰면 아래와 같음
            val obj1 = TestClass()
            obj1.data1 = parcel.readInt()
            obj1.data2 = parcel.readString()
            return obj1
        }
